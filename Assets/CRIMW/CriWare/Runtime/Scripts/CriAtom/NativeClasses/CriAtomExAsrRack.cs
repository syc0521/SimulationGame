...
public partial class CriAtomExAsrRack : CriDisposable
{
	 ...
	 * <remarks>
	 * <para header='Description'>Specifies speaker mapping for CriAtomExAsrRack.<br/></para>
	 * </remarks>
	...
	public CriAtomExAsrRack(Config config, PlatformConfig platformConfig)
	{
		...
		this._rackId = CRIWARE6158351D(ref config, ref platformConfig);
	...
	#if CRIWARE_SUPPORT_ASR
		if (existingRackId < 0) {
			throw new Exception("Invalid rack id.");
		}
	#endif
	...
	private static extern Int32 CRIWARE6158351D([In] ref Config config, [In] ref PlatformConfig platformConfig);
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	...
	#else
	private static Int32 CRIWARE6158351D([In] ref Config config, [In] ref PlatformConfig platformConfig) { return 0; }
