...
public static class CriAtomPlugin
{
...
#endif
	/* @endcond */
		CRIWARE009CE561(max_virtual_voices,
		...
		int communication_buffer_size, int playback_position_update_interval)
	{
		...
		CRIWARE8384362B((uint)max_preview_objects,
	...
	{
		CRIWARE237CA5E3(enable_user_pcm_output);
	...
	{
		CRIWARE2F034134(enable_sonicsync, buffering_time_ios, override_ipod_music_ios);
	}
															 ...
															 int stream_type)
	{
		...
		CRIWARE70981A20(enable_sonicsync,
		...
		if (use_fast_mixer == true) {
			...
			using (AndroidJavaObject activity = jc.GetStatic<AndroidJavaObject>("currentActivity")) {
				...
				CRIWARE7BEB921F(android_context);
			}
		}
		CRIWARE27B3EB48(use_aaudio);
#endif
	}
	...
	public static void SetConfigAdditionalParameters_VITA(int num_atrac9_memory_voices, int num_atrac9_streaming_voices, int num_mana_decoders)
	{
		...
		CRIWARE128C046B(num_atrac9_memory_voices, num_atrac9_streaming_voices, num_mana_decoders);
		#endif
	}
														 ...
														 bool use_audio3d, int num_audio3d_memory_voices, int num_audio3d_streaming_voices)
	{
		...
		CRIWAREA2755500(num_atrac9_memory_voices, num_atrac9_streaming_voices,
	...
	public static void SetConfigAdditionalParameters_PS5(int max_channel_ports, int max_object_ports)
	{
		...
		CRIWARE8529503B(max_channel_ports, max_object_ports);
		#endif
	}
	...
	public static void SetConfigAdditionalParameters_SWITCH(bool enable_sonicsync, int num_opus_memory_voices,
		...
		CRIWARE3D78027D(enable_sonicsync, num_opus_memory_voices, num_opus_streaming_voices, init_socket);
		#endif
	}
	...
	{
		#if UNITY_WEBGL
		CRIWARE4FA5F6E5(num_webaudio_voices);
		#endif
	}
	/* @endcond */
	...
	{
		CRIWARE2CFF30B7(sampling_rate_for_memory, sampling_rate_for_streaming);
	}
	...
	{
		return CRIWARE98A6BE5C();
	}

	public static bool IsInitializedForPcmOutput()
	{
	#if UNITY_EDITOR
		return CRIWAREE097EA6E();
	#else
		return false;
	#endif
	}
	...
	{
		ushort ret = CRIWARE5B1C5B3B(CRIATOMUNITY_PARAMETER_ID_LOOP_COUNT);
	...
	public static void DecryptAcb(IntPtr acb_hn, ulong key, ulong nonce)
	{
		...
		CRIWARE5BC8942D(acb_hn, CallbackFromNative, IntPtr.Zero);
		temporalStorage = 0;
	}
	/* @endcond */
	...
	private static extern void CRIWARE009CE561(int max_virtual_voices,
		int max_voice_limit_groups, int max_categories,
		int max_aisacs, int max_bus_sends,
		int max_sequence_events_per_frame, int max_beatsync_callbacks_per_frame,
		int max_cuelink_callbacks_per_frame,
	...
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE8384362B(uint max_preivew_objects,
		uint communication_buffer_size, int playback_position_update_interval);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE237CA5E3(bool enable_user_pcm_out_mode);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE780E6ED2(long buffering_time_pc);

	/* @cond excludele */
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWAREE996B258(int output, int pulse_latency_usec);
	/* @endcond */

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE2F034134(bool enable_sonicsync, uint buffering_time_ios, bool override_ipod_music_ios);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE70981A20(bool enable_sonicsync, 
																				  int num_low_delay_memory_voices, int num_low_delay_streaming_voices,
																				  int sound_buffering_time,        int sound_start_buffering_time,
	...
	#if !UNITY_EDITOR && UNITY_ANDROID
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE7BEB921F(IntPtr android_context);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE27B3EB48(bool flag);
	#endif

	/* @cond excludele */
	#if !UNITY_EDITOR && UNITY_PSP2
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE128C046B(int num_atrac9_memory_voices, int num_atrac9_streaming_voices, int num_mana_decoders);
	#endif

	#if !UNITY_EDITOR && (UNITY_PS4 || UNITY_PS5)
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWAREA2755500(int num_atrac9_memory_voices, int num_atrac9_streaming_voices,
																			  bool use_audio3d, int num_audio3d_memory_voices, int num_audio3d_streaming_voices);
	#endif

	#if !UNITY_EDITOR && UNITY_PS5
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE8529503B(int max_channel_ports, int max_object_ports);
	#endif

	#if !UNITY_EDITOR && UNITY_SWITCH
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE3D78027D(bool enable_sonicsync, int num_opus_memory_voices,
																				 int num_opus_streaming_voices, bool init_socket);
	#endif

	#if UNITY_WEBGL
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE4FA5F6E5(int num_webaudio_voices);
	#endif
	/* @endcond */

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE2626EF5B();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern bool CRIWAREC176B113();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE63E1F278();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWAREB64E0C3B(bool pause);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern uint CRIWAREA703719E();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWAREBF97E87F(int code);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWARE15B0037A(IntPtr cbfunc, string separator_string);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWARE28E5ABA4(IntPtr cbfunc);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARECCA1268B();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWARE88175F19(IntPtr cbfunc);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE7581CF8C();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWARE15F30D07(IntPtr cbfunc);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARED1FD92AA();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void CRIWARE2CFF30B7(int sampling_rate_for_memory, int sampling_rate_for_streaming);

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWARE797E0462();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWAREC54D0552();

	/* @cond notpublic */
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern void CRIWARE5BC8942D(IntPtr acb_hn, CallbackFromNativeDelegate func, IntPtr obj);
	/* @endcond */

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	public static extern ushort CRIWARE5B1C5B3B(int id);
	...
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern bool CRIWAREE097EA6E();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern int CRIWAREE320F01B();

	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern int CRIWARE98A6BE5C();

	#else
	private static void CRIWARE009CE561(int max_virtual_voices,
		int max_voice_limit_groups, int max_categories,
		int max_aisacs, int max_bus_sends,
		int max_sequence_events_per_frame, int max_beatsync_callbacks_per_frame,
		int max_cuelink_callbacks_per_frame,
		...
		int max_faders, int num_buses, bool use_ambisonics,
		IntPtr spatializer_core_interface) { }
	private static void CRIWARE8384362B(uint max_preivew_objects,
		uint communication_buffer_size, int playback_position_update_interval) { }
	private static void CRIWARE237CA5E3(bool enable_user_pcm_out_mode) { }
	private static void CRIWARE780E6ED2(long buffering_time_pc) { }
	/* @cond excludele */
	private static void CRIWAREE996B258(int output, int pulse_latency_usec) { }
	/* @endcond */
	private static void CRIWARE2F034134(bool enable_sonicsync, uint buffering_time_ios, bool override_ipod_music_ios) { }
	private static void CRIWARE70981A20(bool enable_sonicsync, 
																		   int num_low_delay_memory_voices, int num_low_delay_streaming_voices,
																		   int sound_buffering_time,        int sound_start_buffering_time,
																		   bool apply_hw_property,          int stream_type) { }
	#if !UNITY_EDITOR && UNITY_ANDROID
	private static void CRIWARE7BEB921F(IntPtr android_context) { }
	private static void CRIWARE27B3EB48(bool flag) { }
	#endif

	/* @cond excludele */
	#if !UNITY_EDITOR && UNITY_PSP2
	private static void CRIWARE128C046B(int num_atrac9_memory_voices, int num_atrac9_streaming_voices,
																		int num_mana_decoders) { }
	#endif
	#if !UNITY_EDITOR && (UNITY_PS4 || UNITY_PS5)
	private static void CRIWAREA2755500(int num_atrac9_memory_voices, int num_atrac9_streaming_voices,
																		bool use_audio3d, int num_audio3d_memory_voices, int num_audio3d_streaming_voices) { }
	#endif
	#if !UNITY_EDITOR && UNITY_PS5
	private static void CRIWARE8529503B(int max_channel_ports, int max_object_ports) { }
	#endif
	#if !UNITY_EDITOR && UNITY_SWITCH
	private static void CRIWARE3D78027D(bool enable_sonicsync, int num_opus_memory_voices,
																		  int num_opus_streaming_voices, bool init_socket) { }
	#endif
	#if UNITY_WEBGL
	private static void CRIWARE4FA5F6E5(int num_webaudio_voices) { }
	#endif
	/* @endcond */
	private static bool _dummyInitialized = false;
	private static void CRIWARE2626EF5B() { _dummyInitialized = true; }
	public static bool CRIWAREC176B113() { return _dummyInitialized; }
	private static void CRIWARE63E1F278() { _dummyInitialized = false; }
	private static void CRIWAREB64E0C3B(bool pause) { }
	public static uint CRIWAREA703719E() { return 0; }
	public static void CRIWAREBF97E87F(int code) { }
	public static void CRIWARE15B0037A(IntPtr cbfunc, string separator_string) { }
	public static void CRIWARE28E5ABA4(IntPtr cbfunc) { }
	public static void CRIWARECCA1268B() { }
	public static void CRIWARE88175F19(IntPtr cbfunc) { }
	public static void CRIWARE7581CF8C() { }
	public static void CRIWARE15F30D07(IntPtr cbfunc) { }
	public static void CRIWARED1FD92AA() { }
	private static void CRIWARE2CFF30B7(int sampling_rate_for_memory, int sampling_rate_for_streaming) { }
	public static void CRIWARE797E0462() { }
	public static void CRIWAREC54D0552() { }
	/* @cond notpublic */
	public static void CRIWARE5BC8942D(IntPtr acb_hn, CallbackFromNativeDelegate func, IntPtr obj) { }
	/* @endcond */
	public static ushort CRIWARE5B1C5B3B(int id) { return 0; }
#if !UNITY_EDITOR && UNITY_IOS
	public static void criAtomUnity_SleepToDelay_IOS(int milliseconds) { }
#endif
	private static bool CRIWAREE097EA6E() { return false; }
	private static int CRIWAREE320F01B() { return 0; }
	private static int CRIWARE98A6BE5C() { return 0; }
	#endif
	#endregion
	...
	*/
	...
	void OnDestroy()
	{
		...
		if (beatsyncUserCbFunc != null) {
			...
			CriAtomPlugin.CRIWARE88175F19(IntPtr.Zero);
		}
	...
	public static void SetEventCallback(CriAtomExSequencer.EventCbFunc func, string separator)
	{
		...
		CriAtomPlugin.CRIWARE15B0037A(ptr, separator);
	}
	...
	private static void RegisterEventCallbackChain(CriAtomExSequencer.EventCallback func)
	{
		...
		if (eventUserCallback == null) {
			...
			ptr = Marshal.GetFunctionPointerForDelegate(delegateObject);
			CriAtomPlugin.CRIWARE28E5ABA4(ptr);
		}
	...
	private static void UnregisterEventCallbackChain(CriAtomExSequencer.EventCallback func)
	{
		eventUserCallback -= func;
		if (eventUserCallback == null) {
			CriAtomPlugin.CRIWARE28E5ABA4(IntPtr.Zero);
		}
	}
	...
	private static void RegisterBeatSyncCallbackChain(CriAtomExBeatSync.CbFunc func)
	{
		if (beatsyncUserCbFunc == null) {
			...
			CriAtomPlugin.CRIWARE88175F19(ptr);
		}
	...
	private static void RegisterCueLinkCallbackChain(CriAtomEx.CueLinkCbFunc func)
	{
		if (cueLinkUserCbFunc == null) {
			...
			var ptr = Marshal.GetFunctionPointerForDelegate(delegateObject);
			CriAtomPlugin.CRIWARE15F30D07(ptr);
		}
	...
	private static void UnregisterCueLinkCallbackChain(CriAtomEx.CueLinkCbFunc func)
	{
		cueLinkUserCbFunc -= func;
		if (cueLinkUserCbFunc == null) {
			CriAtomPlugin.CRIWARE15F30D07(IntPtr.Zero);
		}
	}
